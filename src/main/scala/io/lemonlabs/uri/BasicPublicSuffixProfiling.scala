...
object BasicPublicSuffixProfiling extends App {
  ...
  val input = Source.fromURL(getClass.getResource("/alexa-top-1million.txt"), "UTF-8").getLines().map(Url.parse)

  def run(uri: Url): Option[String] = {
    uri.publicSuffix
  }
  ...
  println(PublicSuffixSupportImpl.trie.children.size)
}
